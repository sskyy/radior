...
class Bus {
    ...
    if (listener.blockFor) {
      ...
      for (let listenerToBlock of listener.blockFor) {
        if (listenerList.get(listenerToBlock)) {
          util.ensureSet(listenerList.get(listenerToBlock), 'waitFor', listener.id)
        } else {
          //if blockFor target is not in the listener list yet, we need to wait for its insert.
          util.ensureSet(this._blockForWaiting, listenerToBlock, listener.id)
        }
      }
    }

    //if other listener want to blockFor current listener
    if (this._blockForWaiting[ listener.id ]) {
      util.ensureSet(listener, 'waitFor', this._blockForWaiting[ listener.id ])
      this._blockForWaiting.delete(listener.id)
    }

    //save the listener to map
    map.set(listener.event.toString(), listenerList.insert(listener.id, listener, order))
  ...
  fireListeners(event, listeners, eventArgv) {
    ...
    const firePromise = co(function *() {
      ...
      while (listenerCursor.next !== undefined) {
        ...
        if (event.target && ! event.target.has(listener.id)) continue
        //event can disable certain listener
        if (event.disable && event.disable.get(listener.id)) continue
        ...
        if (listener.waitFor) {
          debug.log('calling listener', listener.id, listener.waitFor)
          ...
          //debug.log(listener.id,'must wait for', listener.waitFor, promiseToWait.length)
          ...
          //debug.log('calling none waitFor listener', listener.id)
          if (listener.async) {
        ...
        //console.log('saving result of', listener.id, result)
        results[ listener.id ] = result

        //debug.log( `result of ${listener.id}`,result,result instanceof ListenerResult )
        ...
        if (result.signal.disable !== undefined) {
          ...
          disableNames.forEach(function (disableName) {
            ...
            event.disable.get(disableName).add({ target: disableName, source: listener.id, type: 'call' })
          })
        }
  ...
  storeDisableRecord(listenerNames, fireEventName, listener) {
    ...
    listenerNames.forEach(function (listenerName) {
      ...
      disableMap.get(listenerName).add({ target: listenerName, source: listener.id, type: 'listener' })
    })
  }
  ...
  getListeners(eventName) {
    ...
    for (let item of this._rexEventListenerMap) {
      ...
      if ((new RegExp(rex)).test(eventName)) {
        ...
        listeners.insert(rexEventListener.id, util.clone(rexEventListener), order)
      }
    }
  ...
  snapshot(listener) {
    ...
    const snapshot = util.extend({
      ...
      _listenerRuntimeKey: this._runtime.getListenerRuntimeKey(this._eventRuntimeKey, listener.id)
    }, this)
  ...
  normalizeListener(eventName, listener) {
    ...
    listener.id = listener.module.toString() ? `${listener.module.toString()}.${listener.name}` : listener.name
