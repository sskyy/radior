...
function cloneDeep(item) {
  ...
  var types = [Number, String, Boolean, Date],
      result;
...
function findIndexToInsert(listener, listeners, cacheIndex) {
  ...
  var firstLast = this._firstLastCache[cacheIndex] !== undefined ? this._firstLastCache[cacheIndex] : findIndex(listeners, function (e) {
    return e.last == true;
  });
...
function from(setOrMap) {
  ...
  try {
    for (var _iterator = setOrMap[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
