...
var OrderedList = (function () {
  function OrderedList(list) {
    ...
    list.forEach(function (itemArgs) {
      _this.insert.apply(_this, itemArgs);
    });
  }
  ...
  _createClass(OrderedList, [{
    ...
    key: 'insert',
    value: function insert(key, value, order) {
      order = order || {};
    ...
    value: function applyOrderAround(obj, orderName) {
      var orderKeys = util.from(obj.order[orderName]);
    ...
    value: function lineUp(obj) {
      ...
      waitForKeys = new Set(waitForKeys.filter(function (key) {
        return !_this3._list.has(key);
      }));

      obj._waiting = waitForKeys;

      util.from(obj._waiting).forEach(function (waitForKey) {
